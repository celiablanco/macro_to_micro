"""
Generate the revised main paper as a Word document (.docx) with changes highlighted in red.
Mirrors generate_paper_pdf.py but outputs .docx for Google Docs import.
"""
import os
...
OUTPATH = os.path.join(os.path.dirname(__file__), 'revised_paper.docx')
...
class RunSpec:
    """Specification for a single formatted run of text."""
...
class MarkupParser(HTMLParser):
    """Parse reportlab-style HTML markup into RunSpec objects for python-docx."""
    def __init__(self):
...
def parse_markup(text):
    """Parse an HTML-like markup string into a list of RunSpec objects."""
    parser = MarkupParser()
    parser.feed(text)
...
# ─────────────────────────────────────────────────────────────────────────────
# Markup helpers (same as PDF generator)
# ─────────────────────────────────────────────────────────────────────────────
...
for section in doc.sections:
    section.top_margin = Cm(2.03)    # ~0.8 in
    section.bottom_margin = Cm(2.03)
    section.left_margin = Cm(2.54)   # 1 in
...
def _apply_runs(paragraph, markup_text, font_size=Pt(10)):
    """Parse markup and add formatted runs to an existing paragraph."""
...
def add_para(text, font_size=Pt(10), alignment=WD_ALIGN_PARAGRAPH.JUSTIFY,
             space_after=Pt(6), space_before=Pt(0), left_indent=None,
             right_indent=None):
    """Add a paragraph with formatted runs parsed from HTML-like markup."""
    p = doc.add_paragraph()
...
def add_heading(text, level=1, font_size=None, red_all=False):
    """Add a heading paragraph."""
    if font_size is None:
        font_size = {1: Pt(14), 2: Pt(12), 3: Pt(11)}[level]
    markup = R(text) if red_all else text
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.LEFT
    p.paragraph_format.space_before = Pt(18) if level == 1 else Pt(12)
    p.paragraph_format.space_after = Pt(8) if level == 1 else Pt(6)
    _apply_runs(p, B(markup), font_size)
    return p


def add_equation(text, red_all=False):
    """Add a centered equation-like paragraph."""
    markup = R(text) if red_all else text
    return add_para(markup, font_size=Pt(10),
                    alignment=WD_ALIGN_PARAGRAPH.CENTER,
                    space_before=Pt(6), space_after=Pt(6))


def add_figure(filename, width_inches=4.0):
    """Add a centered figure from the figures directory."""
...
def add_caption(text, font_size=Pt(9)):
    ...
    return add_para(text, font_size=font_size,
                    alignment=WD_ALIGN_PARAGRAPH.JUSTIFY,
                    space_after=Pt(6))


def add_ref(text, font_size=Pt(9)):
...
def add_table_cell(cell, text, font_size=Pt(8)):
...
add_para(B("Abstract"), font_size=Pt(10), alignment=WD_ALIGN_PARAGRAPH.LEFT,
         space_after=Pt(6))
...
add_para(abstract_text, font_size=Pt(10), alignment=WD_ALIGN_PARAGRAPH.JUSTIFY,
         left_indent=Cm(0.45), right_indent=Cm(0.45), space_after=Pt(12))

add_para(f"{B('Word count:')} {R('~4,200')} words, excluding references.", space_after=Pt(3))
add_para(f"{B('Keywords:')} origins of life, emergence, astrobiology, top-down causation"
         f"{R(', transfer entropy, conditional transfer entropy')}", space_after=Pt(3))
add_para(f"{B('Ethical Compliance:')} This article does not present research with ethical considerations",
         space_after=Pt(3))
add_para(f"{B('Conflict of Interest declaration:')} The authors declare that they have no "
         "affiliations with or involvement in any organization or entity with any financial interest "
         "in the subject matter or materials discussed in this Manuscript.", space_after=Pt(6))
...
add_heading("Parameter choices", level=2, red_all=True)
...
# Parameter table
table_rows = [
    [R(B('Parameter')), R(B('Value')), R(B('Rationale'))],
    [R('<i>r</i><sub>min</sub>, <i>r</i><sub>max</sub>'), R('3.9, 4.0'),
     R('Deep chaotic regime; avoids periodic windows (9,14,31)')],
    [R('<i>K</i><sub>0</sub>'), R('100'),
     R('Conventional scale; results invariant to rescaling')],
    [R('<i>A</i>'), R('0\u201360'),
     R('Swept; covers A/K<sub>0</sub> = 0 to 0.6')],
    [R('\u03c9'), R('2\u03c0/2000'),
     R('~4.5 cycles in analysis window; robustness shown in Fig. 1B inset')],
    [R('<i>N</i>'), R('1000'),
     R('Large enough for well-averaged mean field (\u221aN \u2248 31.6)')],
    [R('<i>T</i>'), R('10,000'),
     R('Sufficient length for TE estimation; first 1,000 discarded')],
    [R('Bins'), R('15'),
     R('Standard for histogram-based TE; balances bias and variance (13)')],
    [R('Lag <i>k</i>'), R('2'),
     R('Avoids near-synchronous inflation at k=1; see Fig. S1')],
    [R('Surrogates'), R('100'),
     R('Circular-shift method; sufficient for p &lt; 0.01 threshold')],
]
...
for row in table.rows:
    ...
    row.cells[2].width = Cm(9.2)

# Fill cells
for i, row_data in enumerate(table_rows):
    for j, cell_text in enumerate(row_data):
        add_table_cell(table.rows[i].cells[j], cell_text, Pt(8))
    if i == 0:
        for cell in table.rows[i].cells:
            set_cell_shading(cell, 'E6E6E6')

add_caption(R(B("Table 1.") + " Summary of model and analysis parameters with justifications."))

# ── Information-theoretic measures (formulas stay with Model) ──
add_heading("Information-theoretic measures", level=2, red_all=True)
...
add_heading(R("Unconditional transfer entropy reveals macro-to-micro asymmetry"), level=2)
...
# Insert Figure 1
add_figure('fig1a.png', 3.2)
add_figure('fig1b.png', 3.4)
add_caption(
...
add_figure('fig2a.png', 3.2)
add_figure('fig2b.png', 3.6)
add_caption(
...
# ═════════════════════════════════════════════════════════════════════════════
add_heading(R("Conditional transfer entropy: disentangling environmental forcing"), level=2)
...
add_heading(R("Role of the A/K<sub>0</sub> ratio"), level=2)
...
# Insert A/K0 figure
add_figure('fig_ak0_ratio.png', 3.6)
add_caption(R(
...
add_heading(R("Inter-unit predictability"), level=2)
...
add_heading(R("Data availability"), level=2)
